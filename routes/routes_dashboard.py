...
def _get_membres_stats(conn) -> dict:
    try:
        total       = conn.execute("SELECT COUNT(*) as n FROM users WHERE telegram_id IS NOT NULL").fetchone()["n"]
        actifs_7j   = conn.execute("""
            SELECT COUNT(DISTINCT user_id) as n FROM messages
            WHERE created_at >= ? AND direction = 'inbound'
        """, (_days_ago(7),)).fetchone()["n"]
        nouveaux_7j = conn.execute("SELECT COUNT(*) as n FROM users WHERE created_at >= ?", (_days_ago(7),)).fetchone()["n"]
        inactifs_21j = conn.execute("""
            SELECT COUNT(DISTINCT u.telegram_id) as n FROM users u
            WHERE u.telegram_id NOT IN (
                SELECT DISTINCT user_id FROM messages WHERE created_at >= ? AND direction = 'inbound'
            )
        """, (_days_ago(21),)).fetchone()["n"]
    except Exception:
...
def _get_abonnements_stats(conn) -> dict:
    try:
        actifs = conn.execute("""
...
def _get_ia_stats(conn) -> dict:
    try:
        messages_ia = conn.execute("""
            SELECT COUNT(*) as n FROM messages WHERE direction='outbound' AND answered_by='ia'
        """).fetchone()["n"]
        total_out = conn.execute("SELECT COUNT(*) as n FROM messages WHERE direction='outbound'").fetchone()["n"]
        taux_resolution = round(messages_ia / total_out * 100, 1) if total_out > 0 else 0
        escalades = conn.execute("SELECT COUNT(*) as n FROM messages WHERE requires_admin=1").fetchone()["n"]
    except Exception:
...
def _get_trading_stats(conn) -> dict:
    try:
        wr_row = conn.execute("""
            SELECT
        ...
        best  = conn.execute("SELECT pair, result_percent, closed_at FROM signals WHERE status='closed' AND result_percent IS NOT NULL ORDER BY result_percent DESC LIMIT 1").fetchone()
        worst = conn.execute("SELECT pair, result_percent, closed_at FROM signals WHERE status='closed' AND result_percent IS NOT NULL ORDER BY result_percent ASC LIMIT 1").fetchone()
        membres_actifs = conn.execute("SELECT COUNT(DISTINCT user_id) as n FROM trade_journal WHERE participated=1").fetchone()["n"]
        cap_moyen      = conn.execute("SELECT ROUND(AVG(last_cap),2) as n FROM (SELECT user_id, MAX(capital) AS last_cap FROM member_capital GROUP BY user_id) t").fetchone()["n"]
        total_journaux = conn.execute("SELECT COUNT(*) as n FROM trade_journal WHERE participated=1").fetchone()["n"]
        trades_ouverts = conn.execute("SELECT COUNT(*) as n FROM signals WHERE status='open'").fetchone()["n"]
    except Exception:
...
def _get_activite_recente(conn) -> list:
    activites = []
    try:
        trades = conn.execute("""
...
def _get_expirations_proches(conn) -> list:
    try:
        rows = conn.execute("""
