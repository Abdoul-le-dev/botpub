...
async def check_buffer_attached_to_current(rep: ConsistencyReport):
    ...
    # status["attached"] est une STR d'affichage — pour comparer, on lit
    # les champs numériques.
    attached_sid = status.get("session_id")
    attached_ver = status.get("version")
    label = status.get("attached", "aucune")

    if reg is None:
        # Registre vide : buffer devrait être détaché aussi.
        if attached_sid is not None and status["pending"] > 0:
            rep.add("buffer_leaks",
                    f"registry vide mais buffer attaché à {label} "
                    f"avec {status['pending']} lignes en attente")
        return

    if (attached_sid, attached_ver) != (reg.session_id, reg.version):
        rep.add("buffer_wrong_session",
                f"registry=#{reg.session_id}v{reg.version} "
                f"buffer attaché à {label}",
                severity="error")
